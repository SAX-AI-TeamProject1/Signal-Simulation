...
#   parents[0]=camera_node  parents[1]=signal_vision(파이썬 패키지)  parents[2]=signal_vision(패키지 루트)
# parents[1] 인 이유: 벤더 사본인 vision_hand 쪽이 같은 파일을 ROOT=parents[2] 로 잡는데,
# 그 파일들은 한 단계 더 깊어서(vision_hand/inference/predict.py) 결과가 파이썬 패키지 폴더가 된다.
# 즉 여기서 parents[1] 이어야 predict.py 의 DEFAULT_MODEL_PATH 와 같은 파일을 가리킨다.
# (예전엔 둘이 서로 다른 models/ 를 보고 있어서 가중치가 두 군데로 갈렸다.)
