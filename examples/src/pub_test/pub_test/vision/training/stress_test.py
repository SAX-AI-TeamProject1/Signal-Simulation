...
from . import augment as aug # 모듈 자체를 가져와 아래에서 P_*/강도 상수를 일시적으로 덮어쓰기 위함
# train.py와 동일한 데이터 로드·분할·모델·평가 로직을 재사용 (중복 구현 금지) —
# split_dataset이 train.py와 같은 시드(42)를 쓰므로 학습 때와 동일한 test 셋을 다시 얻는다.
from .train import DEFAULT_DATA_DIR, DEFAULT_MODEL_PATH, SignLSTM, evaluate, load_dataset, split_dataset
