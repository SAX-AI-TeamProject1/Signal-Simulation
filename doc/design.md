...
- **The lidar only sees forward, and that is the mount, not the sensor.** It used to sit at `(0.3, 0, 1.1)` on the chassis, which is inside the `mecanum_lift` visual mesh — every one of the 360 rays came back between 0.32 m and 1.43 m and no warehouse wall was ever visible. `gpu_lidar` renders visuals rather than collisions, so clearing the URDF collision box did nothing, and the mesh keeps its `x[-1.07, 0.99]` footprint at every height from 0 to 2.01 m, leaving no clear height inside the body. It now sits on the front bumper at `(1.05, 0, 0.5)` and reaches out to 9.6 m, but the body still blocks the rear: 164 of the 360 rays (46%) hit the robot itself, and the clear arc measured in simulation is −95.8° to +99.8°.
  <!-- 라이다가 앞쪽만 보는데, 그건 센서가 아니라 장착 위치 때문임. 예전에는 chassis 기준 (0.3, 0, 1.1) 이었고 그 자리는 mecanum_lift 외형 메시 안쪽이었음 — 광선 360개가 전부 0.32~1.43m 에서 돌아왔고 창고 벽은 한 번도 보이지 않았음. gpu_lidar 는 collision 이 아니라 visual 을 렌더링해서 맞히므로 URDF collision 박스를 넘긴 것만으로는 소용이 없었고, 그 메시는 높이 0~2.01m 전 구간에서 x[-1.07, 0.99] footprint 를 유지해서 몸통 안에는 트이는 높이가 없음. 지금은 앞 범퍼 (1.05, 0, 0.5) 에 달려 9.6m 까지 보지만 뒤쪽은 여전히 몸통에 막힘: 광선 360개 중 164개(46%)가 로봇 자신에 맞고, 시뮬레이션에서 실측한 트인 구간은 -95.8도 ~ +99.8도임. -->
- **The lidar's field of view is still the full 360° it cannot use.** Narrowing `min_angle` / `max_angle` to the measured clear arc would spend all 360 samples on the useful 190° and roughly double the angular resolution, but it also gives up the rear permanently, which Nav2 and SLAM would rather have. That trade is undecided.
  <!-- 라이다 시야각이 아직 쓰지도 못하는 360도 그대로임. min_angle·max_angle 을 실측한 트인 구간으로 좁히면 샘플 360개를 쓸모 있는 190도에 다 쓰게 되어 각 분해능이 대략 두 배가 되지만, 대신 뒤쪽을 영구히 포기하게 되고 Nav2·SLAM 은 그 뒤쪽을 원함. 이 맞바꿈은 아직 결정하지 않았음. -->
