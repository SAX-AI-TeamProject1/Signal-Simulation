...
`STOP` is the only motion label left. The trained checkpoint emits `idle`, `left_go`, `right_go`, `stop`, so the `come`→`FORWARD` entry in `LABEL_MAP` pointed at something the model could never produce, and `FORWARD` sat there looking defined while being unreachable; both were deleted. Driving is the dispatch route's job. Restoring forward means training `come` upstream and putting one line back into each of `LABEL_MAP` and `LABEL_MOTION`. `inference.validate_labels()` now cross-checks the checkpoint's labels, the collection script's `KNOWN_LABELS` and `LABEL_MAP` at startup and warns — never raises — so the same silent gap cannot form again.
<!-- 속도 라벨은 이제 STOP 하나뿐임. 학습된 체크포인트는 idle, left_go, right_go, stop 을 내보내므로 LABEL_MAP 의 come→FORWARD 항목은 모델이 절대 낼 수 없는 것을 가리키고 있었고, FORWARD 는 정의만 남은 채 도달 불가였음 — 둘 다 지웠음. 주행은 파견 경로가 맡음. 전진을 되살리려면 상류에서 come 을 학습한 뒤 LABEL_MAP 과 LABEL_MOTION 에 각각 한 줄씩 되돌리면 됨. 이제 inference.validate_labels() 가 기동할 때 체크포인트의 라벨·수집 스크립트의 KNOWN_LABELS·LABEL_MAP 을 서로 대조해 경고를 남기므로(예외는 안 던짐), 같은 침묵이 다시 생기지는 않음. -->

A dropped label is not held, on purpose. `twist_mux` drops the gesture source 0.5 s after the last message, but inference republishes a confirmed label about every 71 ms and `SignalStabilizer` carries `release_grace=1.0`, keeping `confirmed_idx` set through a confidence dip — the hold already exists upstream and is longer than the timeout. Latching `STOP` in `command_publisher` would stack a second one on top, and it would be wrong anyway: a signaler lowering their hand means "you may go". Safety while driving belongs to `estop_node` (priority 100).
<!-- 라벨이 끊겨도 일부러 유지하지 않음. twist_mux 는 마지막 메시지로부터 0.5초 뒤 gesture 소스를 버리지만, 추론은 확정된 라벨을 약 71ms 마다 다시 발행하고 SignalStabilizer 는 release_grace=1.0 을 갖고 있어 신뢰도가 잠깐 떨어져도 confirmed_idx 를 유지함 — 유지는 이미 상류에 있고 타임아웃보다 김. command_publisher 에서 STOP 을 물면 그 위에 두 번째 유지를 얹는 셈이고, 애초에 틀린 동작이기도 함: 신호수가 손을 내렸다는 건 "가도 된다"는 뜻임. 주행 중 안전은 estop_node(우선순위 100)의 몫임. -->
...
- **The command interface with the vision repository is still not agreed.** The label set used here (`STOP`, `LEFT`, `RIGHT`) and the topic names are this repo's own draft. Topic naming, the command set, and whether a confidence value travels with the command all need to be settled with the other side.
  <!-- 비전 리포지토리와의 명령 인터페이스가 아직 합의되지 않음. 여기서 쓰는 라벨 집합(STOP, LEFT, RIGHT)과 토픽 이름은 이쪽에서 혼자 만든 초안임. 토픽 이름, 명령 종류, 신뢰도 값을 명령과 함께 보낼지 여부를 상대 쪽과 확정해야 함. -->
- **The LEFT/RIGHT ↔ zone pairing is provisional.** `labels.py` maps `LEFT`→`zone_nw` / `RIGHT`→`zone_ne` using the robot's own left and right while it faces the signaler; the robot and the signaler mirror each other, so if the team settles on the signaler's perspective, swap the two values in `LABEL_DISPATCH`. This is a naming convention to agree on, not a defect — left/right recognition itself tested correct.
  <!-- LEFT/RIGHT 와 존의 짝은 잠정임. labels.py 가 신호수를 마주 본 로봇 자신의 좌우 기준으로 LEFT→zone_nw / RIGHT→zone_ne 로 매핑하는데, 로봇과 신호수는 거울상이라 팀이 신호수 기준으로 정하면 LABEL_DISPATCH 의 두 값만 맞바꾸면 됨. 이건 합의할 명명 규약이지 결함이 아님 — 좌우 인식 자체는 실테스트에서 정상이었음. -->
- **Both dispatch routes come home on the same spine (x = 0), and multi-robot arbitration there is deferred.** With one robot the shared lane costs nothing; with two it is a bottleneck and a head-on risk that `estop_node` would only turn into a deadlock, not schedule. The direction discussed is a central traffic-manager node granting the shared segment to one robot at a time — postponed until a second robot is actually spawned.
  <!-- 두 파견 경로 모두 같은 척추(x=0)로 복귀하는데, 거기서의 다중 로봇 중재는 보류함. 로봇이 한 대면 공용 차선의 비용이 없지만, 두 대면 병목이자 정면 대치 위험이고 estop_node 는 그걸 스케줄링이 아니라 교착으로 만들 뿐임. 논의된 방향은 공용 구간을 한 번에 한 대에게만 허가하는 중앙 교통 관리 노드 — 두 번째 로봇이 실제로 스폰될 때까지 미룸. -->
...
- **The robot body is a stand-in traced off a borrowed mesh, and its mass is the part that is most wrong.** `mecanum_lift_robot.urdf.xacro` is the only body in use, wearing `model://Forklift/base_visual.glb` — the sole file in that model directory, so the collision boxes and the 1.95 × 1.09 × 2.2 m dimensions were authored to match a visual rather than read off a specification. `base_mass` is 25 kg, two orders of magnitude below a real forklift, and it is plainly the retired `robot.urdf.xacro`'s 20 kg nudged up. That number is not cosmetic: inertia, braking distance and traction all follow from it, and the safety margins already tuned on top of it — `max_linear_acceleration` 1.5 in the URDF, `slow_decel` 1.2 in `estop_node` — were chosen for a 25 kg robot. When the real vehicle specification lands, those get recomputed, not just the dimensions.
  <!-- 로봇 차체는 남의 메시를 보고 뜬 대역이고, 그중 가장 틀린 값은 질량임. 실제로 쓰이는 유일한 차체인 mecanum_lift_robot.urdf.xacro 는 model://Forklift/base_visual.glb 를 입고 있는데 그 모델 폴더에 있는 파일은 그 glb 하나뿐임 — 즉 콜리전 박스와 1.95 x 1.09 x 2.2 m 치수는 스펙을 읽은 게 아니라 외형에 맞춰 사람이 적어 넣은 것임. base_mass 는 25 kg 로 실제 지게차보다 두 자릿수 작고, 물러난 robot.urdf.xacro 의 20 kg 를 조금 올린 값임이 뻔히 보임. 이 숫자는 겉모습 문제가 아님: 관성·제동거리·접지력이 전부 여기서 따라 나오고, 그 위에서 이미 조정해 둔 안전 여유 — URDF 의 max_linear_acceleration 1.5, estop_node 의 slow_decel 1.2 — 도 25 kg 로봇을 전제로 고른 값임. 실제 차량 스펙이 나오면 치수만이 아니라 그 값들도 다시 계산해야 함. -->
...
- **`camera_node` does not mirror its frames the way the training pipeline does, and left/right recognition still tested fine — leave it alone.** `extractor.process_frame()` applies `cv2.flip(frame, 1)` before detection, and both `collect.py` and the upstream `predict.py` go through it, so the training sequences came from selfie-view frames. `camera_node` takes a different path: `frame_source.read()` returns the raw capture and `inference.py` calls `extractor.detect()` directly, so no flip is applied. Reading the code that looks like it should swap `left_go` and `right_go`, and `training/augment.py` has no mirror augmentation to absorb it — but it was tested on the real webcam and the robot drove the dispatch route, which exercises exactly those two asymmetric labels end to end. The likely reason is that MediaPipe decides anatomical left and right from how the body looks rather than from which side of the image it is on, so the landmark index assignment flips together with the coordinates and the two cancel. Recorded here because the code path genuinely is asymmetric: someone will find it again, conclude it is a bug, and break working behavior by "fixing" it.
  <!-- camera_node 는 학습 파이프라인처럼 프레임을 좌우 반전하지 않는데도 좌우 인식은 실테스트에서 정상이었음 — 건드리지 말 것. extractor.process_frame() 이 검출 전에 cv2.flip(frame, 1) 을 적용하고 collect.py 와 상류 predict.py 가 둘 다 그 함수를 지나므로 학습 시퀀스는 셀피 뷰 프레임에서 나왔음. camera_node 는 경로가 다름: frame_source.read() 가 원본 캡처를 그대로 돌려주고 inference.py 는 extractor.detect() 를 직접 부르므로 플립이 적용되지 않음. 코드만 읽으면 left_go 와 right_go 가 뒤바뀔 것처럼 보이고 training/augment.py 에 그걸 흡수할 좌우 반전 증강도 없지만, 실제 웹캠으로 시험했을 때 로봇이 파견 경로를 주행했음 — 그 두 비대칭 라벨을 처음부터 끝까지 그대로 거치는 검증임. 이유는 MediaPipe 가 해부학적 좌우를 이미지의 어느 쪽인지가 아니라 몸이 어떻게 보이는지로 판정하기 때문일 가능성이 큼 — 랜드마크 인덱스 배정이 좌표와 함께 뒤집혀 둘이 상쇄됨. 코드 경로가 실제로 비대칭이라 여기 적어 둠: 누군가 이걸 다시 발견해 버그라고 결론 내리고 "고치다가" 잘 되던 동작을 망가뜨릴 것이기 때문임. -->
- **The lidar looks forward only, and both halves of that were settled by measurement.** It used to sit at `(0.3, 0, 1.1)`, inside the `mecanum_lift` visual mesh — all 360 rays came back between 0.32 m and 1.43 m and no warehouse wall was ever visible. `gpu_lidar` renders visuals rather than collisions, so clearing the URDF collision box changed nothing, and the mesh holds its `x[-1.07, 0.99]` footprint from 0 to 2.01 m, leaving no clear height inside the body. Moved to the front bumper at `(1.25, 0, 0.35)` it reaches past 9.9 m, but at 360° the body still ate the rear: 148 of 360 rays (41%) hit the robot, and the measured clear arc was −105.8° to +105.8°. So `min_angle`/`max_angle` are now `±1.5708`, inside that arc — self-hit rays went from 148 to 0 and the angular step from 1° to 0.5°. The rear is given up on purpose, accepted because reversing is not planned; when Nav2 and SLAM want it, the answer is a second lidar on the rear bumper, not widening this one back to 360°, which only ever returned the robot's own body.
  <!-- 라이다가 앞쪽만 보는데, 그 두 가지가 모두 실측으로 결정된 것임. 예전에는 (0.3, 0, 1.1) 이었고 그 자리는 mecanum_lift 외형 메시 안쪽이었음 — 광선 360개가 전부 0.32~1.43m 에서 돌아왔고 창고 벽은 한 번도 보이지 않았음. gpu_lidar 는 collision 이 아니라 visual 을 렌더링해 맞히므로 URDF collision 박스를 치운 것만으로는 소용이 없었고, 그 메시는 높이 0~2.01m 전 구간에서 x[-1.07, 0.99] footprint 를 유지해 몸통 안에는 트이는 높이가 없음. 앞 범퍼 (1.25, 0, 0.35) 로 옮기니 9.9m 넘게 보지만, 360도로 두면 뒤쪽은 여전히 몸통에 먹혔음: 광선 360개 중 148개(41%)가 로봇 자신에 맞았고 실측한 트인 구간은 -105.8도 ~ +105.8도였음. 그래서 min_angle·max_angle 을 그 구간 안인 ±1.5708 로 두었고, 자기 몸에 맞는 광선이 148개에서 0개가 됐으며 각 간격은 1도에서 0.5도가 됐음. 뒤쪽은 의도적으로 포기한 것이고 후진할 계획이 없어서 받아들였음. Nav2·SLAM 이 뒤쪽을 원하게 되면 답은 뒤 범퍼에 라이다를 한 대 더 다는 것이지 이 각을 360도로 되돌리는 게 아님 — 되돌려 봐야 로봇 자기 몸만 돌아왔음. -->
